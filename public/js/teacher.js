...
function setupTabs() {
  ...
  if (firstTab) {
    const firstContent = document.getElementById(
      "tab-" + firstTab.getAttribute("data-tab"),
    );
...
function bindEvents() {
  ...
  document
    .getElementById("btnLoadGrades")
    .addEventListener("click", function (e) {
      e.preventDefault();
      loadGradeStudents();
    });
  document
    .getElementById("btnSaveGrades")
    .addEventListener("click", saveGrades);
  document
    .getElementById("materialForm")
    .addEventListener("submit", saveMaterial);
  document
    .getElementById("annForm")
    .addEventListener("submit", saveAnnouncement);
  document
    .getElementById("btnShowAttHistory")
    .addEventListener("click", showAttendanceHistoryModal);
  document
    .getElementById("closeAttHistory")
    .addEventListener("click", closeAttendanceHistoryModal);

  document
    .getElementById("attHistoryModal")
    .addEventListener("click", function (e) {
...
async function showAttendanceHistoryModal() {
  ...
  document.getElementById("attHistoryTable").innerHTML = data
    .map(
      (row) => `
        <tr>
...
async function loadClasses() {
  ...
  document.getElementById("classTable").innerHTML = classes
    .map(
      (c) => `
  ...
  const options = classes
    .map(
      (c) =>
        `<option value="${c.id}">${escapeHtml(c.ten_lop)} - ${escapeHtml(c.ten_mon || "")}</option>`,
    )
    .join("");
...
async function loadAttendanceStudents() {
  ...
  if (!classId) return;
  const students = (await apiRequest(`/teacher/classes/${classId}/students`))
    .data;
  if (!students || students.length === 0) {
    document.getElementById("attTable").innerHTML =
      '<tr><td colspan="3" style="text-align:center;color:#888;">Lớp này chưa có sinh viên đăng ký</td></tr>';
    return;
  }
  document.getElementById("attTable").innerHTML = students
    .map(
      (s) => `
        <tr>
...
async function saveAttendance() {
  ...
  const rows = Array.from(document.querySelectorAll("#attTable select")).map(
    (el) => ({
...
async function loadAssignments() {
  ...
  if (!classId) return;
  const assignments = (await apiRequest(`/teacher/assignments/${classId}`))
    .data;

  document.getElementById("assignmentTable").innerHTML = assignments
    .map(
      (a) => `
        <tr>
            <td>${a.id}</td>
...
async function saveAssignment(e) {
  ...
  if (!file) {
    showAlert(
      "globalAlert",
      "Vui long dinh kem file de bai de sinh vien tai ve.",
      "error",
    );
    return;
  }
...
async function loadSubmissions(assignmentId) {
  ...
  const subs = (
    await apiRequest(`/teacher/assignments/${assignmentId}/submissions`)
  ).data;

  document.getElementById("submissionTable").innerHTML = subs
    .map((s, idx) => {
      let fileNameCell = "";
...
async function saveSubmissionScore(id) {
  ...
  await apiRequest(`/teacher/submissions/${id}/grade`, {
    method: "POST",
    body: { diem, nhan_xet },
  });
...
async function loadGradeStudents() {
  ...
  const students = (await apiRequest(`/teacher/classes/${classId}/students`))
    .data;
  document.getElementById("gradeTable").innerHTML = students
    .map(
      (s) => `
        <tr>
            <td>${escapeHtml(s.ho_ten)}</td>
...
async function saveGrades() {
  ...
  await apiRequest(`/teacher/grades/${classId}`, {
    method: "PUT",
    body: { danh_sach: rows },
  });
...
async function loadMaterials() {
  ...
  document.getElementById("materialTable").innerHTML = mats
    .map(
      (m) => `
        <tr>
            <td>${m.id}</td>
...
async function saveMaterial(e) {
  ...
  try {
    ...
    showAlert(
      "globalAlert",
      error.message || "Upload tài liệu thất bại",
      "error",
    );
  }
}
...
async function loadAnnouncements() {
  ...
  document.getElementById("annTable").innerHTML = anns
    .map(
      (a) => `
        <tr>
            <td>${a.id}</td>
...
async function saveAnnouncement(e) {
  ...
  await apiRequest(
    id ? `/teacher/announcements/${id}` : "/teacher/announcements",
