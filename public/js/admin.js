...
function renderUsers() {
  ...
  const teacherOptions = ['<option value="">Chọn giảng viên</option>'].concat(
    state.teachers.map(
      (t) => `<option value="${t.id}">${escapeHtml(t.ho_ten)}</option>`,
    ),
  );
...
function renderSubjects() {
  ...
  const options = state.subjects
    .map((s) => `<option value="${s.id}">${escapeHtml(s.ten_mon)}</option>`)
    .join("");
...
function renderClasses() {
  ...
  const classOptions = ['<option value="">Tài liệu chung</option>'].concat(
    state.classes.map(
      (c) => `<option value="${c.id}">${escapeHtml(c.ten_lop)}</option>`,
    ),
  );
...
function renderAnnouncements() {
  ...
  tb.innerHTML = state.announcements
    .map(
      (a) => `
        <tr>
            <td>${a.id}</td>
...
async function saveSubject(e) {
  ...
  await apiRequest(id ? `/admin/subjects/${id}` : "/admin/subjects", {
    method: id ? "PUT" : "POST",
    body,
  });
  e.target.reset();
...
async function saveClass(e) {
  ...
  await apiRequest(id ? `/admin/classes/${id}` : "/admin/classes", {
    method: id ? "PUT" : "POST",
    body,
  });
  e.target.reset();
...
async function saveAnnouncement(e) {
  ...
  await apiRequest(id ? `/admin/announcements/${id}` : "/admin/announcements", {
    method: id ? "PUT" : "POST",
    body,
  });
  e.target.reset();
...
async function deleteUser(id) {
  ...
  await loadUsers();
  showAlert("globalAlert", "Đã xóa người dùng");
}
async function deleteSubject(id) {
  await apiRequest(`/admin/subjects/${id}`, { method: "DELETE" });
  await loadSubjects();
  await loadClasses();
  showAlert("globalAlert", "Đã xóa môn học");
}
async function deleteClass(id) {
  await apiRequest(`/admin/classes/${id}`, { method: "DELETE" });
  await loadClasses();
  showAlert("globalAlert", "Đã xóa lớp học.");
}
