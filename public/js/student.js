...
(async function init() {
  ...
  document
    .getElementById("btnLoadDetail")
    .addEventListener("click", loadClassDetail);
...
async function loadMyClasses() {
  ...
  document.getElementById("myClassTable").innerHTML = myClasses
    .map(
      (c) => `
        <tr>
            <td>${c.id}</td>
...
async function loadAvailableClasses() {
  ...
  document.getElementById("availableTable").innerHTML = rows
    .map(
      (c) => `
        <tr>
            <td>${c.id}</td>
...
async function loadClassDetail() {
  ...
  if (!classId) {
    document.getElementById("detailMaterials").innerHTML =
      '<tr><td colspan="3" style="text-align:center;color:#888;">Bạn chưa đăng ký lớp nào.</td></tr>';
    document.getElementById("detailAssignments").innerHTML =
      '<tr><td colspan="5" style="text-align:center;color:#888;">Bạn chưa đăng ký lớp nào.</td></tr>';
    return;
  }
  ...
  document.getElementById("detailMaterials").innerHTML =
    materials
      ...
      .join("") ||
    '<tr><td colspan="3" style="text-align:center;color:#888;">Chưa có tài liệu.</td></tr>';
  ...
  document.getElementById("detailAssignments").innerHTML =
    ...
    `,
      ...
      .join("") ||
    '<tr><td colspan="5" style="text-align:center;color:#888;">Chưa có bài tập cho lớp này.</td></tr>';
}
...
async function submitAssignment(id) {
  ...
  fd.append("file", file);
  try {
    await apiRequest(`/student/assignments/${id}/submit`, {
      method: "POST",
      body: fd,
    });
...
document
  .getElementById("detailClass")
  .addEventListener("change", loadClassDetail);
document
  .querySelector('[data-tab="class-detail"]')
